    ...
    with torch.no_grad():
        for index, batch in enumerate(targetloader):
           ...
           # output1 = nn.functional.softmax(output1, dim=1)
            output1 = (output1 > 0.3).float() 
            #output2 = model2(image)
            #output2 = nn.functional.softmax(output2, dim=1)
            output_nomask = output1.cpu().numpy()
            mask = output_nomask[0]  # maska ma teraz wartości 0 i 1
            mask_uint8 = (mask).astype(np.uint8)

           # print(mask.shape)
            #output_nomask = Image.fromarray(output_nomask[0] * 255).convert('L')  # * 255 aby przeskalować wartości do 0-255
          ...
          #  output=output1.cpu().numpy()
           ...
           # output = output.transpose(1,2,0)
            #output_nomask=output
           # print(output.size())
           # output_nomask = np.asarray( np.argmax(output, axis=2), dtype=np.uint8 )
            output_col = colorize_mask(mask_uint8)
            output_nomask = Image.fromarray(mask_uint8)    
            name = name[0].split('/')[-1]
