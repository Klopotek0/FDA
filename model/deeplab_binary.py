...
class ResNet101(nn.Module):
    ...
    def forward(self, x, lbl=None, weight=None, ita=1.5):
       ...
       # print(lbl.shape)
        if self.phase == 'train':
            P = F.softmax(x, dim=1)        # [B, 2, H, W]
            logP = F.log_softmax(x, dim=1) # [B, 2, H, W]
            PlogP = P * logP               # [B, 2, H, W]
    ...
    def CrossEntropy2d(self, predict, target, weight=None, size_average=True):
        loss = F.binary_cross_entropy(predict, target, weight=weight, size_average=size_average)
        return loss   
...
def Deeplab(num_classes=21, init_weights=None, restore_from=None, phase='train'):
    ...
    if init_weights is not None:
        ...
        for i in saved_state_dict:
            i_parts = i.split('.')
            print(i_parts)
            if not num_classes == 1 or not i_parts[1] == 'layer5':
