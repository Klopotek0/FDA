...
class GTA5DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale = random.choice([ 1.0, 1.25, 1.5, 1.75, 2.0])
        new_size = (int(resize[0] * scale), int(resize[1] * scale))
        image.resize(new_size, Image.BICUBIC)
        label.resize(new_size, Image.NEAREST)
        ...
        label.crop((left, top, left + new_w, top + new_h))        
        #random flip
     ...
     #   upper= self.resize[1]-self.crop_size[1] THROW OUT FOR SAME RES
